...
/**
 * @author holger
 * This rule forwards the event with the corresponding name if the longitude and latitude are not present"
 */
@RuleInfo(
    name = "ForwardIfCoordinatesNotPresentFO",
    description = "Forwards the event with name {" + PROP_EVENT_NAME + "} if the longitude and latitude are not present.",
    accepts = {
            @EventInfo(
...
@Slf4j
public class ForwardIfCoordinatesNotPresentFO extends BaseRule {

    private static final String CLASS_NAME = ForwardIfCoordinatesNotPresentFO.class.getSimpleName();
