 ...
 */
@RuleInfo(
    name = "CalculateEtaByTypeFO",
    description = "Calculates the eta of each fulfilment plan and fulfilment"
        + " based on the fulfillment type ED {" + PROP_PICK_N_PACK_TIME_LIMIT_SETTING_ED+ "} setting "
        + "SD {" + PROP_PICK_N_PACK_TIME_LIMIT_SETTING_SD + "} setting "
...
@ParamString(name = PROP_PICK_N_PACK_TIME_LIMIT_SETTING_ED, description = "The pick and pack time limit setting for ed")
@ParamString(name = PROP_PICK_N_PACK_TIME_LIMIT_SETTING_SD, description = "The pick and pack time limit setting for sd")
...
@Slf4j
public class CalculateEtaByTypeFO extends BaseRule {

    private static final String CLASS_NAME = CalculateEtaByTypeFO.class.getSimpleName();
    ...
    public void execute(ContextWrapper context) {
        ...
        /////
        String EDpickAndPackTimeLimitSettingName = context.getProp(PROP_PICK_N_PACK_TIME_LIMIT_SETTING_ED);
        String SDpickAndPackTimeLimitSettingName = context.getProp(PROP_PICK_N_PACK_TIME_LIMIT_SETTING_SD);
        ///////
...
////////
        ...
        List<Setting> pickNPackTimeLimitSettingsED = settingService.getSettings(
                EDpickAndPackTimeLimitSettingName, SETTING_CONTEXT_AGENT, locationIds);

        List<Setting> pickNPackTimeLimitSettingsSD = settingService.getSettings(
                SDpickAndPackTimeLimitSettingName, SETTING_CONTEXT_AGENT, locationIds);

/////////

///////
        Map<Integer, String> EDPickNPackTimeLimitSettingsMap = pickNPackTimeLimitSettingsED.stream()
                .collect(Collectors.toMap(Setting::getContextId, Setting::getValue));

        Map<Integer, String> SDPickNPackTimeLimitSettingsMap = pickNPackTimeLimitSettingsSD.stream()
                .collect(Collectors.toMap(Setting::getContextId, Setting::getValue));
////////
            ...
            for (FulfilmentPlanFulfilment fulfilmentPlanFulfilment : fulfilmentPlan.getFulfilments()) {
                if (StringUtils.equals(orderType, ENTITY_SUBTYPE_ORDER_HD)
                    ...
                    log.info("{} - Retrieving pick and pack time limit for location with id:{}, map:{}",
                            logPrefix, locationId, EDPickNPackTimeLimitSettingsMap);
                    log.info("{} - Retrieving pick and pack time limit for location with id:{}, map:{}",
                            logPrefix, locationId, SDPickNPackTimeLimitSettingsMap);

                    String pickNPackTimeLimitSetting;
                    if(fulfilmentPlanFulfilment.getFulfilmentType().equalsIgnoreCase("ED") ||fulfilmentPlanFulfilment.getFulfilmentType().equalsIgnoreCase("BOTH"))
                    {
                         pickNPackTimeLimitSetting = EDPickNPackTimeLimitSettingsMap.get(locationId);
                    }else {
                         pickNPackTimeLimitSetting = SDPickNPackTimeLimitSettingsMap.get(locationId);
                    }
